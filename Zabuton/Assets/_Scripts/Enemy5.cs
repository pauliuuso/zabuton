...
public class Enemy5 : MonoBehaviour
{
    ...
    public GameObject atom_particles;
    private bool readyTofire = false;
    private float fireReadyTimer = 10f;

    void Start()
    {
        tilt = gameObject.GetComponent<Soul>().tilt;
        setPosition();
        fightingTime = Random.Range(20, 40);
        yRotation = gameObject.transform.eulerAngles.y;
        atom_particles.SetActive(false);
    }
    ...
    void FixedUpdate()
    {
        ...
        if (initialized && !dodging)
        {
            ...
            if (Time.time > fireReadyTimer)
            {
                readyTofire = true;
                fireReadyTimer = Time.time + 5;
                nextMove = Time.time + 3f;
            }

            if (readyTofire)
            {
                atom_particles.SetActive(true);
            }
