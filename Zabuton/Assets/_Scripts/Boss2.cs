...
public class Boss2 : MonoBehaviour
{
    ...
    private float nextRocketShot;
    private float nextShot;
    private float shooting;
    public GameObject shootFire;
    private bool rocketsReady = false;
    ...
    void FixedUpdate()
    {
        ...
        if(shooting > Time.time && readyToFire)
        {
            if (nextShot < Time.time) fire();
        }
        else if (shooting > Time.time && rocketsReady)
        {
            if (nextRocketShot < Time.time) fire2();
        }
        else if (shooting < Time.time)
        {
            readyToFire = false;
            rocketsReady = false;
        }
        ...
        if (nextRocketShot < Time.time)
        {
            nextRocketShot = Time.time + rocketCooldown;
        }
        ...
        if (initialized && !dodging)
        {
            ...
            if (Time.time > nextMove || (!movingDown && !movingLeft && !movingRight && !movingUp))
            {
                setPosition();
    ...
    void fire()
    {
        if (!gameObject.GetComponent<Soul>().particle_bolt)
        {
            ...
            GameObject shootFireClone = (GameObject)Instantiate(shootFire, canon1.transform.position, Quaternion.Euler(0f, canon1.transform.localEulerAngles.y, 0f));
            GameObject shootFireClone2 = (GameObject)Instantiate(shootFire, canon2.transform.position, Quaternion.Euler(0f, canon2.transform.localEulerAngles.y, 0f));
    ...
    void fire2()
    {
        GameObject rocketClone = (GameObject)Instantiate(rocket, rocketCanon.transform.position, rocket.transform.rotation);
    }
